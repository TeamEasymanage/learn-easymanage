...
class ErpProductAddedTableDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const ErpProductAddedTableDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _ErpProductAddedTableDataTableState createState() => _ErpProductAddedTableDataTableState(); 
} 
 
class _ErpProductAddedTableDataTableState extends State<ErpProductAddedTableDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "ErpProductAddedTable Data Table"; 
  int noOfCardsPerRow = 2; 
  int noOfFieldsForCard = 2; 
 
  String errorMsg = timeErrStr; 
 
  List<ErpProductAddedTable> erpProductAddedTableList = []; 
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "ErpProductAddedTable List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "ErpProductAddedTable List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : ErpProductAddedTableSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    ErpProductAddedTableService.ErpProductAddedTableSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
      ...
      setState(() { 
        erpProductAddedTableList = getList; 
        _loading = false; 
      }); 
      }  
    }); 
    ...
    print("Calling Service : ErpProductAddedTableViewAll ... "); 
    ErpProductAddedTableService.ErpProductAddedTableViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : ErpProductAddedTableFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    ErpProductAddedTableService.ErpProductAddedTableFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: ErpProductAddedTableDataSource(erpProductAddedTableList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                ...
                DataColumn( 
                    label: const Text('Product Added Column'), onSort: doColumnSort), 
                      const DataColumn( 
                        label: Text( 
                          'Edit/Delete', 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable ErpProductAddedTableCardList = 
        erpProductAddedTableList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(ErpProductAddedTableCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(ErpProductAddedTable e) { 
...
Widget _buildListView() {  
    ...
    if (erpProductAddedTableList.length == 0) { 
    return Column( 
      children: [ 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) =>  
                                  const ErpProductAddedTableForm(), 	 
                                  ), 	 
                                ); 	 
                              }, 	 
                              icon: const Icon( 	 
    ...
    var noOfFields = erpProductAddedTableList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            erpProductAddedTableList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(erpProductAddedTableList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(erpProductAddedTableList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: erpProductAddedTableList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(ErpProductAddedTable oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('ErpProductAddedTable Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _ErpProductAddedTableInfo(foundErpProductAddedTable: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  content: Container( 
                                    child: Column( 
                                      mainAxisSize: MainAxisSize.min, 
                                      children: [ 
                                        Text("Record Id: " +oh.productId.toString() ), 
                                        const Text(""), 
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      onPressed: () { 
                                        String valParam = ""; 
                                        if (oh.productId is String ) { 
                                          valParam = "'"+oh.productId.toString()+"'"; 
                                        } else { 
                                          valParam = oh.productId.toString(); 
                                        } 
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            ErpProductAddedTableDataTable(viewType: dispType, pageNo: 0, searchClause: " productId = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => ErpProductAddedTableEdit( 	 
                      				productIdSelected: oh.productId, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                              icon: const Icon( 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await ErpProductAddedTableService.ErpProductAddedTableDelete( 	 
                                      oh.productId) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  erpProductAddedTableList.sort((item1, item2) => 
	  ...
	  erpProductAddedTableList.sort((item1, item2) => 
		  compareColumn(ascending, item1.productAddedColumn, item2.productAddedColumn)); 
    } 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  ErpProductAddedTableDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class ErpProductAddedTableDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<ErpProductAddedTable> _data; 
 
  ErpProductAddedTableDataSource(List<ErpProductAddedTable> erpProductAddedTableList) { 
    _data = erpProductAddedTableList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(_data[index].productAddedColumn))), 
      DataCell(Row( 	
        mainAxisAlignment: MainAxisAlignment.spaceBetween, 	
        children: [ 	
          Builder(builder: (context) { 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _ErpProductAddedTableInfo(foundErpProductAddedTable: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const ErpProductAddedTableDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _ErpProductAddedTableInfo extends StatelessWidget { 	
	  final ErpProductAddedTable foundErpProductAddedTable; 	
	  const _ErpProductAddedTableInfo({Key? key, required this.foundErpProductAddedTable}) 	
	      : super(key: key); 	
