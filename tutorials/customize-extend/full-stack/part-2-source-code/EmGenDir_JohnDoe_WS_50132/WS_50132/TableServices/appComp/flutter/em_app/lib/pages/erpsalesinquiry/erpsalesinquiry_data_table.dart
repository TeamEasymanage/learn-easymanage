...
class ErpSalesInquiryDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const ErpSalesInquiryDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _ErpSalesInquiryDataTableState createState() => _ErpSalesInquiryDataTableState(); 
} 
 
class _ErpSalesInquiryDataTableState extends State<ErpSalesInquiryDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "ErpSalesInquiry Data Table"; 
  ...
  final double formWidth =   400 + (200 * 6) ; //400 + (200 * NoOfColumns)
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "ErpSalesInquiry List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "ErpSalesInquiry List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : ErpSalesInquirySelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    ErpSalesInquiryService.ErpSalesInquirySelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : ErpSalesInquiryViewAll ... "); 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : ErpSalesInquiryFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    ErpSalesInquiryService.ErpSalesInquiryFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: ErpSalesInquiryDataSource(erpSalesInquiryList), 
                      columns: <DataColumn>[ 
                DataColumn( 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable ErpSalesInquiryCardList = 
        erpSalesInquiryList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(ErpSalesInquiryCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(ErpSalesInquiry e) { 
...
Widget _buildListView() {  
    ...
    if (erpSalesInquiryList.length == 0) { 
    return Column( 
      children: [ 
    ...
    var noOfFields = erpSalesInquiryList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            erpSalesInquiryList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(erpSalesInquiryList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(erpSalesInquiryList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: erpSalesInquiryList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(ErpSalesInquiry oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('ErpSalesInquiry Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _ErpSalesInquiryInfo(foundErpSalesInquiry: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  content: Container( 
                                    child: Column( 
                                      mainAxisSize: MainAxisSize.min, 
                                      children: [ 
                                        Text("Record Id: " +oh.dateofinquiry.toString() ), 
                                        const Text(""), 
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      onPressed: () { 
                                        String valParam = ""; 
                                        if (oh.dateofinquiry is String ) { 
                                          valParam = "'"+oh.dateofinquiry.toString()+"'"; 
                                        } else { 
                                          valParam = oh.dateofinquiry.toString(); 
                                        } 
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            ErpSalesInquiryDataTable(viewType: dispType, pageNo: 0, searchClause: " dateofinquiry = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => ErpSalesInquiryEdit( 	 
                      				dateofinquirySelected: oh.dateofinquiry, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                              icon: const Icon( 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await ErpSalesInquiryService.ErpSalesInquiryDelete( 	 
                                      oh.dateofinquiry) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  erpSalesInquiryList.sort((item1, item2) => 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  ErpSalesInquiryDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class ErpSalesInquiryDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<ErpSalesInquiry> _data; 
 
  ErpSalesInquiryDataSource(List<ErpSalesInquiry> erpSalesInquiryList) { 
    _data = erpSalesInquiryList; 
  } 
          ...
          Builder(builder: (context) { 	
            return IconButton( 	
                onPressed: () { 	
                  Navigator.of(context).push(MaterialPageRoute( 	
                    builder: (context) => ErpSalesInquiryEdit( 	
                      dateofinquirySelected: _data[index].dateofinquiry, 	
                    ), 	
                  )); 	
                }, 	
                icon: const Icon( 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _ErpSalesInquiryInfo(foundErpSalesInquiry: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const ErpSalesInquiryDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _ErpSalesInquiryInfo extends StatelessWidget { 	
	  final ErpSalesInquiry foundErpSalesInquiry; 	
	  const _ErpSalesInquiryInfo({Key? key, required this.foundErpSalesInquiry}) 	
	      : super(key: key); 	
