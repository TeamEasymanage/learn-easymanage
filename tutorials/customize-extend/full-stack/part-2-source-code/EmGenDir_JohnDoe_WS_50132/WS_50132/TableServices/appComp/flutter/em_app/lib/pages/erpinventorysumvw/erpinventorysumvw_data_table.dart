...
import 'package:em_app/pages/erpinventorysumvw/erpinventorysumvw_service.dart'; 
import 'package:em_app/pages/erpinventorysumvw/erpinventorysumvw_lib.dart'; 
...
class ErpInventorySumVwDataTable extends StatefulWidget { 
  final String? viewType; 
  final int? pageNo; 
  final String? searchClause; 
  final String? sortClause; 
  const ErpInventorySumVwDataTable({Key? key, this.viewType  
  , this.pageNo 
  , this.searchClause 
  , this.sortClause 
  }) : super(key: key); 
  ...
  @override 
  _ErpInventorySumVwDataTableState createState() => _ErpInventorySumVwDataTableState(); 
} 
 
class _ErpInventorySumVwDataTableState extends State<ErpInventorySumVwDataTable> { 
  String dispType = "DataTable"; 
  String emTitle =  "ErpInventorySumVw Data Table"; 
  int noOfCardsPerRow = 2; 
  int noOfFieldsForCard = 2; 
 
  String errorMsg = timeErrStr; 
 
  List<ErpInventorySumVw> erpInventorySumVwList = []; 
  ...
  final double formWidth =   400 + (200 * 3) ; //400 + (200 * NoOfColumns)
  ...
  void initState() { 
    ...
    if (widget.viewType != null ) { 
      ...
      switch (widget.viewType) { 
        case "ListView": { 
        dispType = "ListView"; 
        emTitle = "ErpInventorySumVw List View"; 
        } 
        break; 
        case "ListCard": { 
        dispType = "ListCard"; 
        emTitle = "ErpInventorySumVw List Card"; 
        } 
        break; 
        default: 
      } 
    ...
    print("Calling Service : ErpInventorySumVwSelectWhere ... "); 
    print("searchClause: [" +searchFilter +"]");  
    print("sortClause: [" +sortBy +"]");  
    ErpInventorySumVwService.ErpInventorySumVwSelectWhere( 
      currPageNo 
      , searchFilter 
      , sortBy 
    ).then((getList) { 
    ...
    print("Calling Service : ErpInventorySumVwViewAll ... "); 
    ErpInventorySumVwService.ErpInventorySumVwViewAll( 
      currPageNo 
    ).then((getList) { 
      if (getList is String) { 
        setState(() { 
 ...
 /* ---------- To Implement - GET value from Hive and FindByColumnName -------------------  
  ...
  bool _loadingProvider = true; 
    ...
    _loading = true; 
    print("Calling Service : ErpInventorySumVwFindByColumnName ... "); 
    if (userCode.isNotEmpty) {
    ErpInventorySumVwService.ErpInventorySumVwFindByColumnName(userCode).then((getList) {
      if (getList is String) {
        setState(() {
          errorMsg = getList;
        });
...
Container getDataTableView() { 
          ...
          return Container(  
                   ...
                   //------------------------------------ Scrollbars 	
                      ...
                      source: ErpInventorySumVwDataSource(erpInventorySumVwList), 
                      columns: <DataColumn>[ 
                DataColumn( 
                    label: const Text('Year'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Month'), onSort: doColumnSort), 
                DataColumn( 
                    label: const Text('Total Qty'), onSort: doColumnSort), 
                      const DataColumn( 
                        label: Text( 
                          'Edit/Delete', 
...
// --------------- List Card View ------------------- 
  GridView getListCard() { 
    Iterable ErpInventorySumVwCardList = 
        erpInventorySumVwList.map((e) =>  
     SizedBox( 
    ...
    return GridView.count( 
        ...
        children: List.from(ErpInventorySumVwCardList) 
    ); 
  } 
 
  Widget getColumnsForListCard1(ErpInventorySumVw e) { 
...
Widget _buildListView() {  
    ...
    if (erpInventorySumVwList.length == 0) { 
    return Column( 
      children: [ 
    ...
    var noOfFields = erpInventorySumVwList[0].toMapLabelVal().length;  
    //print (noOfFields);  
    ...
    dataColumns.add(  
              DataColumn( 
                label: Text(  
                            erpInventorySumVwList[0].toListLabVal()[i][0].toString(),  
    ...
    return DataTable(  
      columns: dataColumns, 
      /* <DataColumn>[  
        DataColumn(label: Text(erpInventorySumVwList[0].toListLabVal()[0][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
        DataColumn(label: Text(erpInventorySumVwList[0].toListLabVal()[1][0].toString(), style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600))),  
      ],  
      */ 
      rows: erpInventorySumVwList  
          .map((e) =>  
              getDataRow(e) 
          ).toList(),  
    );  
...
DataRow getDataRow(ErpInventorySumVw oh) { 
                      ...
                      children: [ 
                        IconButton( 	 
                            onPressed:  () => showDialog<String>( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  title: Text('ErpInventorySumVw Details', 	 
                                      style: TextStyle( 	 
                                          color: Colors.red.shade400, 	 
                                          //fontSize: 22, 	 
                                          //fontWeight: FontWeight.w400 	 
                                          )), 	 
                                  content: _ErpInventorySumVwInfo(foundErpInventorySumVw: oh), 	 
                                  actions: <Widget>[ 	 
                                    TextButton( 	 
                        ...
                        IconButton( 	 
                                ...
                                builder: (BuildContext context) => AlertDialog(  
                                  ...
                                  content: Container( 
                                    child: Column( 
                                      mainAxisSize: MainAxisSize.min, 
                                      children: [ 
                                        Text("Record Id: " +oh.year.toString() ), 
                                        const Text(""), 
                                  ...
                                  actions: <Widget>[ 	 
                                    ...
                                    TextButton( 	  
                                      onPressed: () { 
                                        String valParam = ""; 
                                        if (oh.year is String ) { 
                                          valParam = "'"+oh.year.toString()+"'"; 
                                        } else { 
                                          valParam = oh.year.toString(); 
                                        } 
                                      Navigator.of(context).push(MaterialPageRoute( 	  
                                            builder: (context) =>   
                                            ErpInventorySumVwDataTable(viewType: dispType, pageNo: 0, searchClause: " year = "+valParam+" ",),  
                                            ), 
                                          ); 
                                      }, 
                                      child: const Text('Go'), 
                            ...
                            IconButton( 	 
                              onPressed: () { 	 
                                Navigator.of(context).push(MaterialPageRoute( 	 
                                  builder: (context) => ErpInventorySumVwEdit( 	 
                      				yearSelected: oh.year, 	
                                  ), 	 
                                )); 	 
                              }, 	 
                              icon: const Icon( 	 
                        ...
                        actions: <Widget>[ 	 
                          ...
                          TextButton( 	 
                            onPressed: () async { 	 
                              ...
                              String apiMessage = ""; 	 
                              // Begin Service Call 	 
                              await ErpInventorySumVwService.ErpInventorySumVwDelete( 	 
                                      oh.year) 	 
                                  .then((message) { 	 
                                    apiMessage = message;	 
...
// --------------- DataTable Functions ------------------- 
  void doColumnSort(int columnIndex, bool ascending) { 
	if (columnIndex == 0) { 
	  erpInventorySumVwList.sort((item1, item2) => 
		  compareColumn(ascending, item1.year, item2.year)); 
	} else if (columnIndex == 1) { 
	  erpInventorySumVwList.sort((item1, item2) => 
		  compareColumn(ascending, item1.month, item2.month)); 
	} else if (columnIndex == 2) { 
	  erpInventorySumVwList.sort((item1, item2) => 
		  compareColumn(ascending, item1.totalQty, item2.totalQty)); 
    } 
...
void goPage(int pageNoNext) { 
                          ...
                          //} else { 
                            ...
                            Navigator.pushAndRemoveUntil( 
                              context, 
                              MaterialPageRoute(  
                                      builder: (context) =>  ErpInventorySumVwDataTable(viewType: dispType, pageNo: pageNoNext, searchClause: searchFilter, sortClause: sortBy,), 
                                    ), 
                              (Route<dynamic> route) => route.isFirst 
                              ); 
...
class ErpInventorySumVwDataSource extends DataTableSource { 
  // Generate some made-up data 
  @required 
  late List<ErpInventorySumVw> _data; 
 
  ErpInventorySumVwDataSource(List<ErpInventorySumVw> erpInventorySumVwList) { 
    _data = erpInventorySumVwList; 
  } 
  ...
  DataRow getRow(int index) { 
	...
	DataCell(Text(getEmStrOpt(getEmFmtNumOpt(_data[index].year)))), 
	DataCell(Text(getEmStrOpt(getEmFmtNumOpt(_data[index].month)))), 
	DataCell(Text(getEmStrOpt(_data[index].totalQty.toString()))), //DataCell(Text(getEmStrOpt(getEmFmtCurOpt(_data[index].totalQty)))), 
      DataCell(Row( 	
        mainAxisAlignment: MainAxisAlignment.spaceBetween, 	
        children: [ 	
          Builder(builder: (context) { 	
            ...
            return IconButton( 	
                onPressed: () => showDialog<String>( 	
                      ...
                      builder: (BuildContext context) => AlertDialog( 	
                        ...
                        content: _ErpInventorySumVwInfo(foundErpInventorySumVw: _data[index]), 	
                        actions: <Widget>[ 	
                          TextButton( 	
                            onPressed: () => Navigator.pop(context, 'Cancel'), 	
                            child: const Text('Cancel'), 	
                          ...
                          TextButton( 	
                            onPressed: () async { 	
                              ...
                              context,  
                              MaterialPageRoute(   
                                      builder: (context) =>  const ErpInventorySumVwDataTable(), //default: Disp type PaginatedDataTable   
                                    ),  
                              (Route<dynamic> route) => route.isFirst  
                              ) 
                              : null; 	
...
// --------------- DataTable Delete Call Functions - View Record -------------------	
	...
	class _ErpInventorySumVwInfo extends StatelessWidget { 	
	  final ErpInventorySumVw foundErpInventorySumVw; 	
	  const _ErpInventorySumVwInfo({Key? key, required this.foundErpInventorySumVw}) 	
	      : super(key: key); 	
	 	
	  @override 	
	  Widget build(BuildContext context) { 	
			return ErpInventorySumVw_dispForm(context, foundErpInventorySumVw); 	
	  } 	
	} 	
